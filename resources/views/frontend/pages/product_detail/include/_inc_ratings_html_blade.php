        ...
        <form action="" id="form-review" method="POST">
            ...
            <div>
                <p style="margin-bottom: 0">
                    ...
                    <!-- @if (!empty($ratingsDashboard) && isset($ratingsDashboard[0]['r_number']))
                        @php
                            $ratingNumber = $ratingsDashboard[0]['r_number'];  // Lấy giá trị đầu tiên
                        @endphp
                        @for ($i = 1; $i <= 5; $i++)
                            <i class="la la-star {{ $i <= $ratingNumber ? 'active' : '' }}" data-i="{{ $i }}"></i>
                        @endfor
                    @else

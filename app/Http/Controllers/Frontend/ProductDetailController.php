...
class ProductDetailController extends FrontendController
{
    public function getProductDetail(Request $request, $slug)
    {
        ...
        if ($id) {
            ...
            $productUrl = url("san-pham/mobile/" . (Str::slug($product->slug ?: $product->pro_name)) . "-{$product->id}");

            // Tạo mã QR cho URL
            $qrCode = QrCode::size(200)->generate($productUrl);
            ...
            // ->select(\DB::raw('count(r_number) as count_number'), \DB::raw('sum(r_number) as total'))
            // ->addSelect('r_number')
            // ->toSql(); // Xem câu lệnh SQL
        // dd($ratingsDashboard);


                if (empty($ratingsDashboard)) {
                    $ratingsDashboard[] = ['count_number' => 0, 'total' => 0, 'r_number' => 0];
                }
    ...
    public function generateQrCode($id)
{
    ...
    if ($id) {
        ...
        // Tạo URL cho trang chi tiết sản phẩm
        $productUrl = url("san-pham/{$product->slug}-{$product->id}");

        // Tạo mã QR cho URL
        $qrCode = QrCode::size(200)->generate($productUrl);
    
      // Đường dẫn lưu trữ hình ảnh QR code
      $qrCodePath = public_path('uploads/qrcodes/' . $product->id . '.png'); 

      // Lưu hình ảnh mã QR
      $qrcode->writeFile($qrCodePath);

      // Trả về đường dẫn đến hình ảnh
      return asset('uploads/qrcodes/' . $product->id . '.png'); 
